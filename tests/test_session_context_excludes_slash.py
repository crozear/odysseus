...
def _session_with_slash():
    ...
    s.add_message(ChatMessage("user", "/setup openrouter", metadata={"source": "slash"}))
    s.add_message(ChatMessage("assistant", "Starting OpenRouter sign-in...", metadata={"source": "slash"}))
...
def test_context_excludes_slash_messages():
    ...
    assert "/setup openrouter" not in contents
...
def test_history_still_keeps_slash_messages_for_display():
    ...
    assert any(m.content == "/setup openrouter" for m in s.history)
