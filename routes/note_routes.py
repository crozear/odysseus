...
async def dispatch_reminder(
    ...
    if cache_key:
        try:
            ...
            if last:
                ...
                if should_skip and channel in ("ntfy"):
    ...
    # whose due_date fires while the user has the app open got TWO ntfy
    # (frontend-fired here + background-fired by ping_notes 0–5 min later).
    if ( ntfy_sent or browser_sent or local_browser_sent) and note_id:
        try:
            import json as _json
            ...
            try:
                ...
                _cache = {}
            sent_channel = "ntfy" if ntfy_sent else "browser"
...
def setup_note_routes(task_scheduler=None):
    ...
    async def fire_reminder(request: Request):
        ...
        LLM synthesis line and/or sends an ntfy notification.
        Returns {synthesis}.
        """
