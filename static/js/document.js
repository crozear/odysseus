...
import * as Modals from './modalManager.js';
  ...
  export function openPanel() {
    ...
    pane.innerHTML = `
      ...
      <div class="doc-md-toolbar" id="doc-md-toolbar" style="display:none">
        <div class="md-toolbar-items" id="md-toolbar-items">
          ...
          <button type="button" class="md-dd-toggle" data-dd="code" title="Code">\`<svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg></button>
      ...
      <div id="doc-actions-footer" class="doc-actions-bar">
        <span class="doc-action-split" id="doc-copy-export-split">
          <button type="button" id="doc-footer-copy-btn" class="doc-action-btn doc-action-main" title="Copy document"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Copy</button>
          <button type="button" id="doc-footer-export-btn" class="doc-action-btn doc-action-caret" title="Export as…" aria-label="Export options"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 15 12 9 18 15"/></svg></button>
        </span>
      </div>
    ...
    document.getElementById('doc-footer-copy-btn')?.addEventListener('click', () => copyDocument());
    ...
    function _applyDocFont() {
      [editorWrap].filter(Boolean).forEach(el => {
    ...
    if (ta && pre) {
      ...
      function _doFind(dir, focusTextarea) {
        ...
        // visibility across markdown / code modes.
      ...
      // a field) deletes the active document. Keeps the keyboard shortcut
  ...
  export function injectFreshDoc(doc) {
    ...
    // is composed from another full-screen view, `isOpen` can be stale-true while the
  ...
  // pane was torn down by another full-screen view: in that case openPanel()
  // early-returns and nothing mounts, so the doc silently never appears. Reset
  // the stale flag and re-open for real.
  ...
  export async function loadSessionDocs(sessionId, opts = {}) {
    ...
    // but keep session-less docs — they're independent
    for (const [id, doc] of [...docs]) {
  ...
  // markdown and any other mode regardless of single-layer-
  ...
  function renderFindRects(matches, currentIdx) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      // mirror measurement that varies across markdown/code modes.
  ...
  function _isCodeDoc() {
    ...
    const prose = new Set(['markdown', 'md', 'text', 'txt', 'html', 'csv']);
  ...
  export function handleDocUpdate(data) {
    ...
    const isEdit = isExistingDoc && oldContent && oldContent !== newContent && !streamingId;
    if (isEdit && textarea) {
